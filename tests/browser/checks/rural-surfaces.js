...
export async function checkRuralSurfaces() {
  ...
  await generateSite({name: 'rural-surface-test', size: {w: 1200, h: 1200},
    ...
    roads: [road,walk], linear_features: [rail], buildings: [], areas: [], pois: [], extras: []},
  ...
  for (let i = 0; i < ground.index.count; i += 3) {
    ...
    if (Math.max(...xs) >= 518 && Math.min(...xs) <= 587 && Math.max(...zs) >= 503 && Math.min(...zs) <= 577) nearby.push(...ids);
  }
  ...
  let samples = 0;
  try {
    for (const [feature, mesh] of [[road,meshes.asphalt],[walk,meshes.pavement],[rail,ballast]]) {
